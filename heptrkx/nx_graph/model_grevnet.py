...
class SegmentClassifier(snt.Module):
  ...
  def __init__(self, name="SegmentClassifier"):
    ...
    self._core_s = [make_in(), make_in()]
    self._core_t = [make_in(), make_in()]
  ...
  def __call__(self, input_op, num_processing_steps):
    ...
    x0, x1 = tf.split(latent.nodes, num_or_size_splits=2, axis=1)
    x0 = latent.replace(nodes=x0)
    x1 = latent.replace(nodes=x1)


    output_ops = []
    for _ in range(num_processing_steps):
      s = self._core_s[0](x0).nodes
      t = self._core_t[0](x0).nodes
      x1 = x1.replace(nodes=x1.nodes * tf.math.exp(s) + t)

      s = self._core_s[1](x1).nodes
      t = self._core_t[1](x1).nodes
      x0 = x0.replace(nodes=x0.nodes * tf.math.exp(s) + t)

    latent = latent.replace(nodes=tf.concat([x0.nodes, x1.nodes]), axis=1)
