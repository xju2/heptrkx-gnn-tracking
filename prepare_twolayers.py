...
def select_segments(hits1, hits2, phi_slope_max, z0_max, deta_max):
    """
    ...
    good_seg_mask = (phi_slope.abs() < phi_slope_max) & (z0.abs() < z0_max) & (deta.abs() < deta_max)
    return hit_pairs[['index_1', 'index_2']][good_seg_mask]

def construct_graph(hits, layer_pairs,
                    phi_slope_max, z0_max, deta_max,
    ...
    for (layer1, layer2) in layer_pairs:
        ...
        segments.append(select_segments(hits1, hits2, phi_slope_max, z0_max, deta_max))
    # Combine segments from all layer pairs
    segments = pd.concat(segments)
    print("Segments: {}".format(segments.shape[0]))

    # Prepare the graph matrices
    n_hits = hits.shape[0]
    n_edges = segments.shape[0]
    print("hits: {}, Edges: {}".format(n_hits, n_edges))
...
def process_event(prefix, output_dir, pt_min, phi_slope_max, z0_max, deta_max):
    # Load the data
    evtid = int(prefix[-9:])
    ...
    logging.info('Event %i, finished constrcuting graphs' % evtid)
    logging.info('Graphs {}'.format(len(graphs_all)))
