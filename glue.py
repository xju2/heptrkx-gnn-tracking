...
def create_glue(graph, weights, hit_ids, hits, truth):
    ...
    def glue(outer_layer, inner_layer):
        ...
        logging.debug("# of reciprocal choice: {}".format(len(good_match.keys())))
        logging.debug("# of to-be-determined:  {}".format(len(to_be_det_idx.keys())))
        logging.debug("# of good outer exp.:   {}".format(len(res_outer_expolate)))
        ...
        logging.debug("unique match:     {}".format( count.shape[0]) )
        logging.debug("duplicated match: {}".format( repeated_out_hits.shape[0]) )
        ...
        logging.debug("# of unique edges: {}".format( vals.shape[0]) )
        logging.debug("# of repeated    : {}".format( vals[np.where(count > 1)].shape[0]) )
        logging.debug("# of total edges:  {}".format( len(all_edges)) )
        logging.debug("# of positive:     {}".format( n_true_pos_edge) )
        logging.debug("# of true edges:   {}".format( n_true_edge) )
        logging.debug("precision:         {}".format( n_true_pos_edge/n_true_edge) )
...
if __name__ == "__main__":
    ...
    logging.info("precision: {}".format( np.sum(weights*G.y)/G.y.nonzero()[0].shape[0] ) )
    ...
    for ilayer in range(n_det_layers-1):
        ...
        end_time = time.time()
        logging.info("{} to {} takes: {:.1f} ms with precision {:.4f}".format(
            outer_layer, inner_layer, (end_time - start_time)*1000, precision)
        )


    # save these pairs and precision
    logging.info("total pairs: {}".format( sum([len(x) for x in all_pairs])) )
